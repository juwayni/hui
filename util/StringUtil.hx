    ...
    public static function formatNumber(n:Float, precision:Int = 0, standardNotation:Bool = true, includeSpace:Bool = false):String {
        ...
        if (standardNotation) {
            ...
            if (suffix.length != 0) {
                ...
                if (p == -1 && precision > 0) {
                    p = s.length;
                    s += haxe_ui.locale.Formats.decimalSeparator;
                }
            ...
            if (decimalSeparatorIndex == -1) {
                s = humanReadableRegex.replace(s, haxe_ui.locale.Formats.thousandsSeparator);
            } else {
                var integerPart = s.substring(0, s.indexOf("."));
                var decimalPart = s.substring(s.indexOf(".") + 1);
                s = humanReadableRegex.replace(integerPart, haxe_ui.locale.Formats.thousandsSeparator);
                s += haxe_ui.locale.Formats.decimalSeparator;
                s += decimalPart;
            }
        }
