...
import haxe_ui.core.IDirectionalComponent;
import haxe_ui.core.InteractiveComponent;
import haxe_ui.layouts.Layout;
import haxe_ui.layouts.LayoutFactory;
...
import haxe_ui.parsers.ui.resolvers.AssetResourceResolver;
import haxe_ui.parsers.ui.resolvers.ResourceResolver;
...
class RuntimeComponentBuilder {
    ...
    private static function buildComponentFromInfo(c:ComponentInfo):Component {
        ...
        if ((component is haxe_ui.containers.ScrollView)) { // special properties for scrollview and derived classes
            var scrollview:haxe_ui.containers.ScrollView = cast(component, haxe_ui.containers.ScrollView);
        ...
        if ((component is IDataComponent) && c.data != null) {
            cast(component, IDataComponent).dataSource = new haxe_ui.data.DataSourceFactory<Dynamic>().fromString(c.dataString, haxe_ui.data.ArrayDataSource);
        }
    ...
    private static function buildValidators(c:ComponentInfo, component:Component, validators:Array<ValidatorInfo>) {
        ...
        for (validator in validators) {
            ...
            var instance = haxe_ui.validators.ValidatorManager.instance.createValidator(type);
...
interface IComponentDelegate {
    public var component(get, set):haxe_ui.core.Component;
}
#end