...
import haxe_ui.backend.kha.ImageCache;
import haxe_ui.backend.kha.KeyboardHelper;
import haxe_ui.backend.kha.MouseHelper;
import haxe_ui.backend.kha.ScissorHelper;
import haxe_ui.backend.kha.StyleHelper;
...
import haxe_ui.util.MathUtil;
import kha.Color;
import kha.graphics2.Graphics;
import kha.graphics2.ImageScaleQuality;
import kha.input.KeyCode;

@:access(haxe_ui.backend.ImageDisplayImpl)
class ComponentImpl extends ComponentBase {
    ...
    public function renderTo(g:Graphics) {
        ...
        if (this is haxe_ui.components.Canvas) {
            if (batch) {
    ...
    private function checkIncrementBatchBreaker(child:Component) {
        if ((child is haxe_ui.containers.windows.Window) || (child is haxe_ui.containers.dialogs.Dialog)) {
    ...
    // Events
    //***********************************************************************************************************
    @:access(haxe_ui.backend.TextInputImpl)
    ...
    private function __onMouseMove(event:MouseEvent) {
        ...
        if (i == true) {
            if (hasComponentOver(cast this, x, y) == true) {
                var fn:UIEvent->Void = _eventMap.get(haxe_ui.events.MouseEvent.MOUSE_OUT);
                if (fn != null) {
                    var mouseEvent = new haxe_ui.events.MouseEvent(haxe_ui.events.MouseEvent.MOUSE_OUT);
            ...
            var fn:UIEvent->Void = _eventMap.get(haxe_ui.events.MouseEvent.MOUSE_MOVE);
            if (fn != null) {
                var mouseEvent = new haxe_ui.events.MouseEvent(haxe_ui.events.MouseEvent.MOUSE_MOVE);
                mouseEvent.screenX = x / Toolkit.scaleX;
                mouseEvent.screenY = y / Toolkit.scaleY;
                fn(mouseEvent);
            }
        ...
        if (i == true && _mouseOverFlag == false) {
            ...
            _mouseOverFlag = true;
            var fn:UIEvent->Void = _eventMap.get(haxe_ui.events.MouseEvent.MOUSE_OVER);
            if (fn != null) {
                var mouseEvent = new haxe_ui.events.MouseEvent(haxe_ui.events.MouseEvent.MOUSE_OVER);
                mouseEvent.screenX = x / Toolkit.scaleX;
                mouseEvent.screenY = y / Toolkit.scaleY;
                fn(mouseEvent);
            }
    ...
    private function __onMouseDown(event:MouseEvent) {
        ...
        if (i == true && _mouseDownFlag == false) {
            ...
            var type = button == 0 ? haxe_ui.events.MouseEvent.MOUSE_DOWN: haxe_ui.events.MouseEvent.RIGHT_MOUSE_DOWN;
            var fn:UIEvent->Void = _eventMap.get(type);
            if (fn != null) {
                var mouseEvent = new haxe_ui.events.MouseEvent(type);
                mouseEvent.screenX = x / Toolkit.scaleX;
                mouseEvent.screenY = y / Toolkit.scaleY;
                fn(mouseEvent);
            }
    ...
    private function __onMouseUp(event:MouseEvent) {
        ...
        if (i == true) {
            ...
            if (_mouseDownFlag == true) {
                var type = button == 0 ? haxe_ui.events.MouseEvent.CLICK: haxe_ui.events.MouseEvent.RIGHT_CLICK;
                var fn:UIEvent->Void = _eventMap.get(type);
                if (fn != null) {
                    var mouseEvent = new haxe_ui.events.MouseEvent(type);
                    mouseEvent.screenX = x / Toolkit.scaleX;
                ...
                if (type == haxe_ui.events.MouseEvent.CLICK) {
            ...
            var type = button == 0 ? haxe_ui.events.MouseEvent.MOUSE_UP: haxe_ui.events.MouseEvent.RIGHT_MOUSE_UP;
            var fn:UIEvent->Void = _eventMap.get(type);
            if (fn != null) {
                var mouseEvent = new haxe_ui.events.MouseEvent(type);
                mouseEvent.screenX = x / Toolkit.scaleX;
    ...
    private function __onDoubleClick(event:MouseEvent) {
        ...
        if (i == true && button == 0) {
            ...
            if (_lastClickTimeDiff < 0.5 && mouseDelta < 5) { // 0.5 seconds
                var type = haxe_ui.events.MouseEvent.DBL_CLICK;
                var fn:UIEvent->Void = _eventMap.get(type);
                if (fn != null) {
                    var mouseEvent = new haxe_ui.events.MouseEvent(type);
                    mouseEvent.screenX = x / Toolkit.scaleX;
