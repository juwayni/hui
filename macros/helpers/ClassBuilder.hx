...
class ClassBuilder {
    ...
    //  * once we get to 'haxe_ui.core.ComponentCommon' we know we dont care anymore
    //  * we can cache all fields from 'haxe_ui.core.Component' and upwards
	...
	static public function findFieldEx(c:ClassType, name:String):Null<ClassField> {
        var fullName = c.pack.join(".") + "." + c.name;
        if (fullName == "haxe_ui.core.Component") {
            if (CLASS_FIELD_CACHE != null) {
                var map = CLASS_FIELD_CACHE.get(fullName);
                if (map != null) {
                    return map.get(name);
        ...
        if (fullName == "haxe_ui.core.ComponentCommon") {
            return field;
        }
