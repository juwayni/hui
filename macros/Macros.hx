...
import haxe_ui.macros.ComponentMacros.NamedComponentDescription;
import haxe_ui.macros.ModuleMacros;
import haxe_ui.macros.helpers.ClassBuilder;
import haxe_ui.macros.helpers.CodeBuilder;
import haxe_ui.macros.helpers.CodePos;
import haxe_ui.macros.helpers.FieldBuilder;
...
class Macros {
    ...
    private static function buildPropertyBinding(builder:ClassBuilder, f:FieldBuilder, variable:Expr, field:String) {
        ...
        if (hasSetter == true) {
            builder.ctor.add(macro {
                $i{variable}.registerEvent(haxe_ui.events.UIEvent.CHANGE, function(e) {
    ...
    static function buildClonable(builder:ClassBuilder) {
        ...
        var useSelf:Bool = (builder.fullPath == "haxe_ui.backend.ComponentBase");
    ...
    static function buildBehaviours():Array<Field> {
                ...
                if (f.name == valueField) {
                    if (f.isDynamic == true) {
                        newField = builder.addSetter(f.name, f.type, macro { // add a normal (Variant) setter but let the binding manager know that the value has changed
                            ...
                            dispatch(new haxe_ui.events.UIEvent(haxe_ui.events.UIEvent.PROPERTY_CHANGE, $v{f.name}));
                            dispatch(new haxe_ui.events.UIEvent(haxe_ui.events.UIEvent.PROPERTY_CHANGE, "value"));
                            return value;
                        }, f.access);
                    } else {
                        newField = builder.addSetter(f.name, f.type, macro { // add a normal (Variant) setter but let the binding manager know that the value has changed
                            ...
                            switch (Type.typeof(value)) {
                                case TClass(String):
                                    if (value != null && value.indexOf("{{") != -1 && value.indexOf("}}") != -1) {
                                        haxe_ui.locale.LocaleManager.instance.registerComponent(cast this, $v{f.name}, value);
                                        return value;
                                    }
                                case _:    
                            }
                            if (behaviours == null) {
                                behaviours = new haxe_ui.behaviours.Behaviours(cast this);
            ...
            if (resolvedValueField != null && (resolvedValueField.isVariant || 
                ...
                if (resolvedValueField.isString) {
                    builder.addSetter(f.name, macro: Dynamic, macro {
                        switch (Type.typeof(value)) {
                            case TEnum(haxe_ui.util.Variant.VariantType):
                                var v:haxe_ui.util.Variant = value;
                                $i{propName} = v;
                    ...
                    builder.addSetter(f.name, macro: Dynamic, macro {
                        $i{propName} = haxe_ui.util.Variant.fromDynamic(value);
                        return value;
                    }, false, true);
                }
            } else {
        ...
        if (builder.hasInterface("haxe_ui.core.IClonable") && !builder.isAbstractClass) {
