...
import haxe_ui.core.ComponentClassMap;
import haxe_ui.core.ComponentFieldMap;
import haxe_ui.core.TypeMap;
import haxe_ui.macros.ModuleMacros;
import haxe_ui.macros.helpers.ClassBuilder;
import haxe_ui.macros.helpers.CodeBuilder;
import haxe_ui.macros.helpers.FunctionBuilder;
import haxe_ui.parsers.ui.ComponentInfo;
import haxe_ui.parsers.ui.ComponentParser;
import haxe_ui.parsers.ui.LayoutInfo;
import haxe_ui.parsers.ui.ValidatorInfo;
import haxe_ui.parsers.ui.resolvers.FileResourceResolver;
import haxe_ui.util.EventInfo;
import haxe_ui.util.ExpressionUtil;
import haxe_ui.util.SimpleExpressionEvaluator;
import haxe_ui.util.StringUtil;
import haxe_ui.util.TypeConverter;
import sys.io.File;
...
@:access(haxe_ui.macros.Macros)
class ComponentMacros {
    @:deprecated("'haxe_ui.macros.ComponentMacros.build' is deprecated, use 'haxe_ui.ComponentBuilder.build' instead")
    ...
    @:deprecated("'haxe_ui.macros.ComponentMacros.buildComponent' is deprecated, use 'haxe_ui.ComponentBuilder.fromFile' instead")
    ...
    @:deprecated("'haxe_ui.macros.ComponentMacros.buildComponentFromString' is deprecated, use 'haxe_ui.ComponentBuilder.fromString' instead")
    ...
    macro public static function cascadeStylesTo(id:Expr, styleProperties:Expr = null, recursiveFind:Null<Bool> = null):Expr {
        ...
        if (recursiveFind != null) {
            builder.add(macro var c = _component.findComponent($e{id}, haxe_ui.core.Component, $v{recursiveFind}));
        } else {
            builder.add(macro var c = _component.findComponent($e{id}, haxe_ui.core.Component));
        }
        var propertyExprs = [];
    ...
    private static function buildCommon(resourcePath:String, params:Expr = null):Array<Field> {
        ...
        if (builder.hasSuperClass("haxe_ui.core.Component") == false) {
            Context.error("Must have a superclass of haxe_ui.core.Component", Context.currentPos());
        }
        #end
        ...
        #if !haxeui_dont_impose_base_class
        if (haxe_ui.util.RTTI.hasSuperClass(builder.fullPath, rootType) == false) {
    ...
    private static function buildComponentFromFile(classBuilder:ClassBuilder, builder:CodeBuilder, filePath:String, buildData:BuildData = null, rootVarName:String = "this", buildRoot:Bool = true):ComponentInfo {
        ...
        for (s in c.styles) {
            if (s.scope == "global") {
                builder.add(macro haxe_ui.Toolkit.styleSheet.parse($v{s.style}, "user"));
            }
        }
        ...
        if (buildRoot == false) {
            if (classBuilder.hasInterface("haxe_ui.core.IDataComponent") == true && c.data != null) {
    ...
    private static function buildComponentFromStringCommon(builder:CodeBuilder, source:String, buildData:BuildData = null, rootVarName:String = "this", buildRoot:Bool = false, classBuilder:ClassBuilder = null):ComponentInfo {
        ...
        buildScriptHandlers(builder, buildData.namedComponents, buildData.scripts);
        if (classBuilder != null && classBuilder.hasInterface("haxe_ui.core.IDataComponent") == true && c.data != null) {
    ...
    private static function assignLanguageBinding(builder:CodeBuilder, languageBinding:LanguageBindingData, namedComponents:Map<String, NamedComponentDescription>, addLocalVars:Bool = false) {
        ...
        for (dependantName in dependants.keys()) {
            ...
            for (dependantProp in propList) {
                ifBuilder.add(macro if (e.data == $v{dependantProp}) {
                    //haxe_ui.locale.LocaleManager.instance.refreshFor($i{varName});
                    haxe_ui.locale.LocaleManager.instance.refreshAll();
                });
            }
            ...
            builder.add(macro {
                $i{generatedDependantName}.registerEvent(haxe_ui.events.UIEvent.PROPERTY_CHANGE, function(e:haxe_ui.events.UIEvent) {
                    $e{ifBuilder.expr}
                });
            });
        }
        ...
        builder.add(macro haxe_ui.locale.LocaleManager.instance.registerComponent($i{varName}, $v{field}, function() {
            return $e{expr};
        }));
    ...
    private static function buildScriptHandlers(builder:CodeBuilder, namedComponents:Map<String, NamedComponentDescription>, scripts:Array<ScriptData>) {
        ...
        for (sh in scripts) {
            if (sh.eventName != null && sh.generatedVarName != null) {
                ...
                var eventType = "haxe_ui.events.UIEvent";
    ...
    private static function replaceInternalShortNames(e:Expr):Expr {
        return switch (e.expr) {
            case EConst(CIdent(s)):
                var r = e;
                if (s == "theme") {
                    r = macro haxe_ui.themes.ThemeManager.instance;
                }
                r;
            case _:
    ...
    private static function buildComponentNode(builder:CodeBuilder, c:ComponentInfo, id:Int, parentId:Int, buildData:BuildData, recurseChildren:Bool = true) {
        ...
        if (classInfo.hasSuperClass("haxe_ui.core.ItemRenderer")) { // we dont really want to create variable instances of contents of item renderers
            useNamedComponents = false;
        }
        if (classInfo.hasDirectInterface("haxe_ui.core.IDirectionalComponent")) {
        ...
        if (c.styles.length > 0) {
            builder.add(macro $i{componentVarName}.styleSheet = new haxe_ui.styles.StyleSheet());
            for (s in c.styles) {
                if (s.scope == "local") {
        ...
        if (classInfo.hasInterface("haxe_ui.core.IDataComponent") == true && c.data != null) {
        ...
        if (c.id != null && buildData.namedComponents != null && useNamedComponents == true) {
            ...
            if (rootClassInfo.hasField(c.id, true) == false) {
                ...
                if (classInfo.hasSuperClass("haxe_ui.components.Column") == false) {
    ...
    private static function buildDataSourceCode(builder:CodeBuilder, c:ComponentInfo, dsVarName:String, componentVarName:String) {
        var items = new haxe_ui.data.DataSourceFactory<Dynamic>().fromStringToArray(c.dataString);
        builder.add(macro var $dsVarName = new haxe_ui.data.ArrayDataSource<Dynamic>());
        for (item in items) {
        ...
        builder.add(macro ($i{componentVarName} : haxe_ui.core.IDataComponent).dataSource = $i{dsVarName});
    }
    ...
    private static function buildLayoutCode(builder:CodeBuilder, c:ComponentInfo, componentVarName:String) {
        ...
        var layoutClass = haxe_ui.layouts.LayoutFactory.lookupClass(layoutName);
        if (layoutClass == null) {
    ...
    private static function buildValidatorCode(builder:CodeBuilder, validators:Array<ValidatorInfo>, id:Int) {
        ...
        for (validator in validators) {
            ...
            validatorExprs.push(macro @:mergeBlock {
                var $validatorId = haxe_ui.validators.ValidatorManager.instance.createValidator($v{type});
            });
        ...
        for (propName in properties.keys()) {
            ...
            if (StringTools.startsWith(propName, "on")) {
                ...
                if (c != null && c.resolvedClassName != null) {
                    var propType = null;
                    var propInfo = haxe_ui.util.RTTI.getClassProperty(c.resolvedClassName, propName);
                    if (propInfo != null) {
    ...
    private static function assignBinding(builder:CodeBuilder, bindingData:BindingData, namedComponents:Map<String, NamedComponentDescription>, addLocalVars:Bool = false) {
        ...
        for (dependantName in dependants.keys()) {
            if (dependantName == "theme") { // special case for themes - may need to expand on this for other parts of core
                builder.add(macro {
                    var theme = haxe_ui.themes.ThemeManager.instance;
                    haxe_ui.themes.ThemeManager.instance.registerEvent(haxe_ui.events.ThemeEvent.THEME_CHANGED, function(_) {
