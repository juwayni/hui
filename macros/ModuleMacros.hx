...
import haxe_ui.macros.ComponentMacros.BuildData;
import haxe_ui.macros.helpers.ClassBuilder;
import haxe_ui.macros.helpers.CodeBuilder;
import haxe_ui.parsers.modules.Module;
import haxe_ui.parsers.modules.ModuleParser;
...
class ModuleMacros {
        ...
        */
        ...
        for (m in _modules) {
            ...
            if (m.preloader != null) {
                var p = m.preloader;
                builder.add(macro
                    haxe_ui.HaxeUIApp.instance.preloaderClass = cast Type.resolveClass($v{p})
                );
            }
            ...
            for (t in m.themeEntries) {
                if (t.parent != null) {
                    builder.add(macro
                        haxe_ui.themes.ThemeManager.instance.getTheme($v{t.name}).parent = $v{t.parent}
                    );
                }
                for (r in t.styles) {
                    ...
                    if (useResource) {
                        builder.add(macro
                            haxe_ui.themes.ThemeManager.instance.addStyleResource($v{t.name}, $v{r.resource}, $v{r.priority}, $v{r.styleData})
                        );
                    }
                }
                for (r in t.images) {
                    builder.add(macro
                        haxe_ui.themes.ThemeManager.instance.addImageResource($v{t.name}, $v{r.id}, $v{r.resource}, $v{r.priority})
                    );
                }
                for (r in t.vars.keys()) {
                    var v = t.vars.get(r);
                    builder.add(macro
                        haxe_ui.themes.ThemeManager.instance.setThemeVar($v{t.name}, $v{r}, $v{v})
                    );
                }
            }
            ...
            for (p in m.properties) {
                builder.add(macro
                    haxe_ui.Toolkit.properties.set($v{p.name}, $v{p.value})
                );
            }

            for (p in m.preload) {
                builder.add(macro
                    haxe_ui.ToolkitAssets.instance.preloadList.push({type: $v{p.type}, resourceId: $v{p.id}})
                );
            }
                ...
                if (c.loadAll == true) { // loadAll means will be populate the classmap - this means a ref will be made to EACH of these components
                    ...
                    if (types != null) {
                        for (t in types) {
                            ...
                            if (classInfo.hasSuperClass("haxe_ui.core.Component")) {
                                var fullPath = classInfo.fullPath;
                                builder.add(macro
                                    haxe_ui.core.ComponentClassMap.register($v{classInfo.name}, $v{fullPath})
                                );
                            } else if (classInfo.hasInterface("haxe_ui.IComponentDelegate")) {
                                var fullPath = classInfo.fullPath;
                                builder.add(macro
                                    haxe_ui.core.ComponentClassMap.register($v{classInfo.name}, $v{fullPath})
                                );
            ...
            for (l in m.locales) {
                ...
                for (r in l.resources) {
                    if (r != null) {
                        builder.add(macro
                            haxe_ui.locale.LocaleManager.instance.parseResource($v{localeId}, $v{r})
                        );
                    }
                }
            }
            ...
            for (validator in m.validators) {
                ...
                builder.add(macro
                    haxe_ui.validators.ValidatorManager.instance.registerValidator($v{id}, function() {
                        return new $t();
            ...
            for (inputSource in m.actionInputSources) {
                ...
                builder.add(macro
                    haxe_ui.actions.ActionManager.instance.registerInputSource(new $t())
                );
            }
            ...
            for (imageLoader in m.imageLoaders) {
                ...
                builder.add(macro
                    haxe_ui.loaders.image.ImageLoader.instance.register($v{imageLoader.prefix}, function() {
                        return new $t();
            ...
            for (cssFunction in m.cssFunctions) {
                builder.add(macro
                    haxe_ui.styles.CssFunctions.registerCssFunction($v{cssFunction.name}, $p{cssFunction.call.split(".")})
                );
            }
            ...
            for (cssFilter in m.cssFilters) {
                ...
                builder.add(macro
                    haxe_ui.styles.CssFilters.registerCssFilter($v{cssFilter.name}, $p{ctor.split(".")})
                );
            }
            ...
            for (cssDirective in m.cssDirectives) {
                ...
                builder.add(macro
                    haxe_ui.styles.DirectiveHandler.registerDirectiveHandler($v{cssDirective.name}, $p{ctor.split(".")})
                );
            }
        }
        ...
        if (preloadAll) {
            for (r in _resourceIds) {
                if (isImage(r)) {
                    builder.add(macro
                        haxe_ui.ToolkitAssets.instance.preloadList.push({type: "image", resourceId: $v{r}})
                    );
                } else if (isFont(r)) {
                    builder.add(macro
                        haxe_ui.ToolkitAssets.instance.preloadList.push({type: "font", resourceId: $v{r}})
                    );
                }
            }
        }
        ...
        for (alias in ComponentClassMap.list()) {
            builder.add(macro
                haxe_ui.core.ComponentClassMap.register($v{alias}, $v{ComponentClassMap.get(alias)})
            );
        }
    ...
    private static function resolveComponentClassInternal(t:haxe.macro.Type, c:ModuleComponentEntry, namespace:String) {
        ...
        if (builder.hasSuperClass("haxe_ui.core.Component") == true) {
            ...
            if (builder.hasInterface("haxe_ui.core.IDirectionalComponent")) {
    ...
    public static function createDynamicClass(filePath:String, alias:String = null, root:String = null, namespaces:Map<String, String> = null):String {
        ...
        */
        ...
        var superClassString = "haxe_ui.containers.Box";
    ...
    private static function processLayoutEntries(layoutEntries:Array<ModuleLayoutEntry>, builder:CodeBuilder = null) {
        for (l in layoutEntries) {
            ...
            if (types != null) {
                for (t in types) {
                    var classInfo = new ClassBuilder(t);
                    if (classInfo.hasSuperClass("haxe_ui.layouts.Layout")) {
                        var fullPath = classInfo.fullPath;
                        for (alias in LayoutMacros.buildLayoutAliases(fullPath)) {
                            if (builder != null) {
                                builder.add(macro haxe_ui.layouts.LayoutFactory.register($v{alias}, $v{fullPath}));
                            }
                            haxe_ui.layouts.LayoutFactory.register(alias, fullPath);
                        }
                    }
                }
            }
